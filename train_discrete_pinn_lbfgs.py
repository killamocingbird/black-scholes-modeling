...
xtrain0 = d0[:9 * len(dataset) // 10, :-1]
ytrain0 = d0[:9 * len(dataset) // 10, -1]
xtrain1 = d1[:9 * len(dataset) // 10, :-1]
ytrain1 = d1[:9 * len(dataset) // 10, -1]

xval0 = d0[9 * len(dataset) // 10:, :-1]
yval0 = d0[9 * len(dataset) // 10:, -1]
xval1 = d1[9 * len(dataset) // 10:, :-1]
yval1 = d1[9 * len(dataset) // 10:, -1]
...
for epoch in range(epochs):
    ...
    model.train()
    def closure():
        loss = (L2(model.get_u_0(xtrain0, r), ytrain0.expand(RK_order, len(ytrain0)).t()) + 
                L2(model.get_u_1(xtrain1, r), ytrain1.expand(RK_order, len(ytrain1)).t()))
    ...
    running_val_loss = 0  
    model.eval()
    running_val_loss = (L2(model.get_u_0(xval0, r), yval0.expand(RK_order, len(yval0)).t()) + 
                        L2(model.get_u_1(xval1, r), yval1.expand(RK_order, len(yval1)).t()))
